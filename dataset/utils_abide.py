...
root_folder = './'
data_folder = r'D:\Code\Dataset\ho\ho_871\ABIDE_pcp\cpac\filt_noglobal'
phenotypic_folder = r'D:\Code\Dataset\ho\ho_871/ABIDE_pcp/Phenotypic_V1_0b_preprocessed1.csv'
sid_folder = r'D:\Code\Dataset\ho\ho_871\subject_IDs_871.txt'
# sid_folder = "D:\Code\Dataset\subject_IDs_1035.txt"
...
def load_all_networks(subject_list, kind, atlas_name="ho"):
    """
...
def feature_selection(features, labels, train_ind, fnum):
    ...
    featureX = features[train_ind, :]   # (533, 110, 110)
    featureY = labels[train_ind]        # (533,)
    # print('featureX', featureX.shape)
    # print('featureY', featureY.shape)
    n_samples, dim1, dim2 = featureX.shape
    featureX1 = featureX.reshape(n_samples, dim1 * dim2)    # (533, 12100)
    # print('featureX1', featureX1.shape)
    selector = RFE(estimator, n_features_to_select=fnum, step=2525, verbose=0)
    selector = selector.fit(featureX1, featureY.ravel())

    n_samples1, dim3, dim4 = features.shape
    features1 = features.reshape(n_samples1, dim3 * dim4)
    x_data = selector.transform(features1)     # (871, 2000)
...
def get_static_affinity_adj(features, pd_dict):  # features.shape = 871 x 2000
    pd_affinity = create_affinity_graph_from_scores(['SITE_ID', 'SEX'], pd_dict)   # 'SEX', 'SITE_ID', 'AGE_AT_SCAN'

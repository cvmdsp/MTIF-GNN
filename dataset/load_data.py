...
# files = ['rois_tt', 'conn_mat']
# files = ['rois_ez', 'conn_mat']
# files = ['rois_dosenbach160', 'conn_mat']

filemapping = {'rois_ho': 'rois_ho.1D',
               'conn_mat': '_' + atlas + '_' + connectivity + '.mat'}
# filemapping = {'rois_tt': 'rois_tt.1D',
#                'conn_mat': '_' + atlas + '_' + connectivity + '.mat'}
# filemapping = {'rois_ez': 'rois_ez.1D',
#                'conn_mat': '_' + atlas + '_' + connectivity + '.mat'}
# filemapping = {'rois_dosenbach160': 'rois_dosenbach160.1D',
#                'conn_mat': '_' + atlas + '_' + connectivity + '.mat'}
...
def save_mat():
    ...
    for s, fname in zip(subject_IDs, fetch_filenames(subject_IDs, files[0])):
        print(f"Subject: {s}, File: {fname}")  # 👈 打印出来看看是哪一个是 "N/A"
        if fname == "N/A":
            print(f"Warning: Subject {s} has no valid filename.")
            continue  # 跳过该 subject，避免崩溃
...
def load_data():
    ...
    save_path = './save_ho/bas_cv'
    os.makedirs(os.path.dirname(save_path), exist_ok=True)
    with open('./save_ho/bas_cv', 'wb') as f:
        pickle.dump(bas, f)
    ...
    with open('./save_ho/site_info_cv', 'wb') as f:
